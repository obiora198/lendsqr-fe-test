...
import './Users.scss'; // Reuse users table styles
...
const Dashboard: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    // For mock purposes, let's say anyone with a loan repayment > 0 has a loan
    const usersWithLoans = users.filter(u => parseFloat(u.education.loanRepayment) > 0).length;
    // For mock purposes, let's say anyone with balance > 20,000 has significant savings
      ...
      <div className="table-container">
        {globalSearch && (
          <div className="search-status-bar">
            ...
            <button onClick={() => navigate('/dashboard')}>Clear</button>
          </div>
        )}
